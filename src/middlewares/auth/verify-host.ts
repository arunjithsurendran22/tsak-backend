 ...
 * @param next
 */
export const verifyHost = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    jwt.verify(token, config.jwt.secret as string, (err: any, payload: any) => {
      ...
      if (payload.role !== 'host') throw new ERROR.AuthorizationError('Authorization Error: Invalid role!');
      req.body.userHostId = payload.sub;
      next();
    });
  } catch (e) {
    next(e);
